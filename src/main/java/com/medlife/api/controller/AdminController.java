...
import com.medlife.api.aop.TrackExecutionTime;
...
public class AdminController {
	...
	public ResponseEntity<Boolean> registerUser(@RequestBody @Valid User user) {
	    ...
	    if (isUserAdded) {
	        return ResponseEntity.ok(true); // Return 200 OK with true if user was added successfully
	    } else {
	        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(false); // Return 400 Bad Request with false if user could not be added
	    }
	}
